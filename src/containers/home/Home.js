...
class Home extends React.Component {
    ...
    render() {
      ...
      const {
        capacityBuildingContentSelected
      } = this.state;
      const {
        awarenessContentSelected
      } = this.state;
      const {
        communityContentSelected
      } = this.state;
      const {
        importedData
      } = this.state;
      ...
      return ( <
        ...
        Header sendToHome = {
          this.selectProjectFromHeader
        }
        contentName = {
          this.state.contentName
        }
        importedProjects = {
          this.state.importedProjects
        }
        />

        {
          ...
          this.state.contentName !== "Main" && ( < FilterTabs sendToHome = {
              this.selectContentFromFilterTabs
            }
            contentName = {
              this.state.contentName
            }
            importedData = {
              importedData[this.state.importedData.projectNames[this.state.projectName]]
            } > < /FilterTabs>)} {
...

          {
            ...
            this.state.loading && ( < CircularProgress id = "loader"
              className = {
                "loader"
              }
              thickness = {
              ...
              !this.state.loading && mappingContentSelected && ( < this.state.MappingContent importedData = {
                  importedData
                }
                />)} {!this.state.loading && capacityBuildingContentSelected && ( < this.state.CapacityBuildingContent importedData = {
                importedData
              }
              />)} {!this.state.loading && awarenessContentSelected && ( < this.state.AwarenessContent importedData = {
              importedData
            }
            />)} {!this.state.loading && communityContentSelected && ( < this.state.CommunityContent importedData = {
            importedData
          }
          />)}
